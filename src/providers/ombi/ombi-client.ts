...
function mapStatusError(status: number): RequestProviderError {
  if (status === 401) {
...
export class OmbiClient {
  ...
  constructor(options: OmbiClientOptions) {
  ...
  private async request<T>(
    ...
    try {
      ...
      if (error instanceof RequestProviderError) {
        throw error;
      }
...
export function createOmbiClientFromEnvironment(): OmbiClient {
  const baseUrl = process.env.OMBI_URL?.trim();
  const apiKey = process.env.OMBI_API_KEY?.trim();
  ...
  return new OmbiClient({ baseUrl, apiKey });
}

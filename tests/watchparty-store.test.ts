...
async function writeStore(parties: unknown[]) {
  await writeFile(
    storePath,
    `${JSON.stringify({ version: 1, parties }, null, 2)}\n`,
    'utf8',
  );
}
...
test('expires an active party six hours after its scheduled start', async () => {
  const scheduledAt = new Date(Date.now() - 6 * 60 * 60 * 1000 - 1000).toISOString();
  await writeStore([
    party({
      scheduledAt,
      status: 'active',
...
test('removes terminal history older than configured retention', async () => {
  ...
  assert.deepEqual(
    raw.parties.map(item => item.id).sort(),
    ['future', 'recent-cancelled'],
  );
});
